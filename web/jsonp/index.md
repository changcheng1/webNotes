...
> cookie LocalStorage都具有同源限制，防止cookie共享带来的安全性
```javaScript
    // 跨域强制携带cookie
...
##2. cors
+ 最常用的方式，后端设置Access-Control-Allow-Origin
...
+ 前提条件:这两个域名必须属于同一个基础域名，而且所用协议，端口都必须一致，否则无法使用documeng.domain进行跨域

+ 前提条件满足之后利用document.domain设置为同一个一级域名就可以实现跨域
```javaScript
